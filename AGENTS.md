...
在處理多行文字疊加時，**絕對禁止**透過命令列的 `text='...'` 參數直接傳遞帶有換行符號 (`\n`) 的字串。
*   **錯誤現象**：如果直接傳遞 `\n`，FFmpeg 雖然會正確斷行，但底層字體渲染引擎 (FreeType) 會試圖畫出 `0x0A` 這個控制字元，導致在每一行末尾畫出一個「無法解析的方塊符號 (Missing Glyph Box)」。
*   **錯誤的修正嘗試**：不可使用 `.replace("\n", "\\n")` 進行跳脫！FFmpeg 的濾鏡解析器會將 `\\n` 直接解析為英文字母 `n`，導致文字不僅不換行，還會在原本該換行的地方印出 `n`。
*   **唯一正確解法 (本專案現行機制)**：一律使用 FFmpeg `drawtext` 提供的 **`textfile`** 參數。程式需先將乾淨的文字 (需手動清空 `\r` 回車符號) 寫入本地暫存文字檔 (如 `.tmp_top_text.txt`)，再將檔案路徑傳給 `textfile='...'`。FFmpeg 的內部讀檔解析器會乾淨地切除換行字元再交給字體引擎，完美避開亂碼問題。
